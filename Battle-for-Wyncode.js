...
function runGame() {
 ...
 [a] ${priceA} gold - Add 200 to your Strength  \n\n [b] ${priceB} gold - Double the magnitude of your next battle effect \n\n [c] ${priceC} gold - Block rivals from stealing gold on your next defeat\n\n [x] Back to Action Select\n\n`);
  ...
  function shuffleBattleEffects() {
    battleEffectsOnTurn = [];
    let allBattleEffects = [
      "Strength +25%",
      "Strength +150",
      "Plunder on Victory x2",
      "Strength -50",
      "Gold +50",
      "Strength -100",
      "Strength -100",
  ...
  function fight(source) {
    ...
    let rivalChoice = prompt.question(
      `Type a rival's name to initiate a Battle.\nType 'x' to return to Action Select.\n\n`
    );
    if (rivalChoice === "x") {
      askForAction();
    }
    if (rivalChoice.toLowerCase() === rivalA.name.toLowerCase()) {
      ...
      battleEffectsOnTurn.forEach((effect) =>
        console.log(
          "[" + (battleEffectsOnTurn.indexOf(effect) + 1) + "] " + effect
        )
      );
      ...
      if (battleEffectsMultiplier === 1) {
        ...
        if (userBattleEffect === "Strength +150") {
          battleStrength += 300;
          console.log(
            `Battle Effects Multiplier Active! ${userBattleEffect} x2! Your strength for this battle is ${battleStrength}.`
          );
        }
        ...
        if (userBattleEffect === "Gold +40") {
          currentGold += 80;
          console.log(
            `Battle Effects Multiplier Active! ${userBattleEffect} x2! You now have ${currentGold} gold.`
          );
        }
    ...
    if (currentOpponent === rivalA) {
      ...
      if (battleStrength > rivalA.strength) {
        currentVictories++;
        let plunderAmount =
          10 + parseInt(rivalA.gold * (Math.floor(Math.random() * 26) / 100));
        if (doublePlunder === false) {
          plunderAmount > rivalA.gold
            ? (plunderAmount = rivalA.gold)
        ...
        rivalA.victories++;
        let plunderAmount =
          5 + parseInt(currentGold * (Math.floor(Math.random() * 26) / 100));
        if (immuneFromPlunder === false) {
          plunderAmount > currentGold
            ? (plunderAmount = currentGold)
            : (plunderAmount = plunderAmount);
          ...
          console.log(
            `Shop item activated! You are protected you from losing any gold.`
          );
  ...
  function fortify(source) {
    if (source === "invalid input") {
      ...
      let possibleFortificationStrings = [
        "+50 Strength",
        "+75 Strength",
        "+100 Strength",
        "+100 Strength",
      ...
      possibleFortificationStrings.forEach((effect) =>
        console.log(
          "[" +
            (possibleFortificationStrings.indexOf(effect) + 1) +
            "] " +
            effect
        )
      );
  ...
  function simulateNPCBattle(NPC1, NPC2) {
    ...
    if (NPCBattleDecider > 2) {
      ...
      let NPC1BattleStrength =
        NPC1.strength + (Math.floor(Math.random() * 3) + 1) * 100;
      let NPC2BattleStrength =
        NPC2.strength + (Math.floor(Math.random() * 3) + 1) * 100;
      ...
      if (NPC1BattleStrength > NPC2BattleStrength) {
        NPC1.victories++;
        let plunderAmount = parseInt(
          NPC2.gold * (Math.floor(Math.random() * 34) / 100)
        );
        ...
        NPC2.victories++;
        let plunderAmount = parseInt(
          NPC1.gold * (Math.floor(Math.random() * 34) / 100)
        );
        NPC2.gold += plunderAmount;
        NPC1.gold -= plunderAmount;
        ...
        console.log(
          "The battle ended in a draw! Neither combatant gains a victory."
        );
      }
    } else {
  ...
  function simulateNPCActionsOnFortification() {
    ...
    if (NPCBattleDecider > 2) {
      ...
      let combatant1Strength =
        combatant1.strength + (Math.floor(Math.random() * 3) + 1) * 100;
      let combatant2Strength =
        combatant2.strength + (Math.floor(Math.random() * 3) + 1) * 100;
      if (combatant1Strength > combatant2Strength) {
        combatant1.victories++;
        let plunderAmount = parseInt(
          combatant2.gold * (Math.floor(Math.random() * 34) / 100)
        );
        ...
        let plunderAmount = parseInt(
          combatant1.gold * (Math.floor(Math.random() * 34) / 100)
        );
        combatant2.gold += plunderAmount;
        combatant1.gold -= plunderAmount;
      ...
      let fortifyingRivalFortification =
